...
export function animate() {
  ...
  if (!isMobile) {
    if (!state.isSceneVisible && !keepSnowRunning) {
      return; // 0fps when not visible (desktop only)
    }

    const frameInterval = (state.isIdle && !keepSnowRunning) ? IDLE_FRAME_INTERVAL : BASE_FRAME_INTERVAL;
    ...
    if (now - state.lastFrameTime < BASE_FRAME_INTERVAL) {
      return;
    }
    state.lastFrameTime = now;
  }
